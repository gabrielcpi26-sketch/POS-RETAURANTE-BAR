...
export default function LoginGate({ children }) {
  ...
  // ✅ PINs por defecto (si no existen en localStorage)
  const adminPin = useMemo(
    () => localStorage.getItem(LS_PIN_ADMIN) || "1234",
    []
  );
  const waiterPin = useMemo(
    () => localStorage.getItem(LS_PIN_WAITER) || "0000",
    []
  );
  ...
  const enter = () => {
    ...
    if (pin !== String(expected)) {
      setError(selected === "admin" ? "PIN de administrador incorrecto" : "PIN de empleado incorrecto");
      setPin("");
      return;
    }
  ...
  const businessName = "El Tridente";
  ...
  const S = {
    ...
    sub: { margin: "6px 0 0", fontSize: 14, color: "rgba(15,23,42,0.65)", fontWeight: 700 },
    logoWrap: {
      borderRadius: 22,
    ...
    accessSub: { margin: "4px 0 0", fontSize: 12, color: "rgba(15,23,42,0.60)", fontWeight: 700 },
    roleTabs: {
      display: "grid",
    ...
    tab: (active) => ({
      ...
      borderRadius: 12,
      border: "1px solid rgba(15,23,42,0.08)",
      background: active ? "rgba(16,185,129,0.16)" : "rgba(255,255,255,0.68)",
      color: "#0f172a",
      cursor: "pointer",
      fontWeight: 1000,
      letterSpacing: 0.6,
      transition: "transform .12s ease, background .18s ease, box-shadow .18s ease",
      boxShadow: active ? "0 12px 26px rgba(16,185,129,0.18)" : "0 10px 18px rgba(2,6,23,0.06)",
      ...
      <div style={S.body}>
        <div style={{ ...S.lockShell }} className="lockShellGrid">
          ...
          <div style={S.right}>
            ...
            <div style={{ fontSize: 11, color: "rgba(15,23,42,0.55)", fontWeight: 800 }}>
